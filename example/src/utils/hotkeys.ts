...
function handleSelectUpperLeftAll(editor: EditorWithBlock, event: KeyboardEvent) {
  const format = editor.detectBlockFormat(containerBlocks);
  if (format) {
    event.preventDefault();
...
function handleSelectLowerRightAll(editor: EditorWithBlock, event: KeyboardEvent) {
  const format = editor.detectBlockFormat(containerBlocks);
  if (format) {
    event.preventDefault();
